...
class cv {
  ...
  let crossHair = UILabel()
    ...
    myImageView.image = #imageLiteral(resourceName: "color wheel.jpg")
...
{  crossHair.frame =   CGRect(x: self.view.bounds.width/2-20, y:self.view.bounds.height/2-20, width: 40, height: 40)
  if uni.ranOnce == false {
     uni.ranOnce = true
        ...
        crossHair.text = "╳"
        crossHair.font = crossHair.font.withSize(40)
  ...
  lineShape.frame = CGRect.init(x: self.view.bounds.width/2-20, y:self.view.bounds.height/2-20, width: 40, height: 40)
...
if uni.ranOnce {
crossHair.frame = CGRect(x: self.view.bounds.width/2-20, y:self.view.bounds.height/2-20, width: 40, height: 40)
lineShape.frame = crossHair.frame 
}
...
/// This function will help find the color of the specified point which is marked using the given pointer
  /// - Parameter gesture: drag the pointer to a point inside the image and obtain the pixel color of the point.
   @objc func colorFinderFunction(_ gesture : UIPanGestureRecognizer) {
     ...
     lineShape.frame = label.frame //  the circular shapelayer moves to x
   ...
   // Start of the RGB ShowCaser
    ...
    values.frame = CGRect(x: myImageView.bounds.width - 80, y: myImageView.bounds.height / 20, width: 100, height: 60)
   ...
   //         print("\(cgp.x) and \(cgp.y) \r \(ow) and \(oh)")
           //let bro = image!.pixel(point: cgp, sourceView: Scroll)
         let bro2 = image?.pixel(point: cgp2, sourceView: myImageView)
         valueDisplayer(bro2 ?? UIColor.black)
         color.backgroundColor = UIColor(cgColor: bro2?.cgColor ?? CGColor.init(srgbRed: 0, green: 0, blue: 0, alpha: 1))
           }
          img()
       }
